...
to initialize
   let randomNumber random-float 100.0
   if randomNumber < 33
      [ setOccupied ]
   if randomNumber >= 33 and randomNumber < 66
    [ setUnoccupied ]
   if randomNumber >= 66
    [ setDegraded ]
end
...
to setDegraded
  set patchState -1
  set pcolor grey
  set regenerationCounter 20
end


to go
  ask patches
    [ set occupied-neighbors count neighbors with [patchState = 1] ]
  degradation
  dying
  ask patches
  [if patchState = 1
    [dispersal]]
  regenerate
  set a array:from-list n-values 150 [0]
  find-clusters
  tick
end
...
to degradation
  ask patches
  [if patchState = 0 and random-float 100.0 < 10
    [setDegraded]]
end

to dying
  ask patches
  [if patchState = 1 and random-float 100.0 < grazing-pressure
    [setUnoccupied]]
end
...
to dispersal
  let localAmount 0
  let globalAmount 0
  set localAmount seed-production-per-patch / 100 * local-positive-interaction
  set localSeeds round localAmount
  set globalAmount seed-production-per-patch - localSeeds
  set globalSeeds round globalAmount
  globalDispersal
  localDispersal
end
...
to globalDispersal
  while [globalSeeds > 1]
    [set globalSeeds globalSeeds - 1
    ask one-of patches
      [if patchState = 0 and random-float 100.0 < 50.0
        [germinate]]]
end

to localDispersal
  while [localSeeds > 1]
    [set localSeeds localSeeds - 1
...
to germinate
  if random-float 100.0 < germination-probability
  [setOccupied]
end

to regenerate
  ask patches
   [let lpiRegeneration 0
    set lpiRegeneration occupied-neighbors * ((local-positive-interaction + 10) / 100)
      if patchState = -1
       [ifelse regenerationCounter <= 0
         [setUnoccupied]
...
"" ""
PENS
"Occupied" 1.0 0 -13840069 true "" "plot count patches with [pcolor = green]"
"Unoccupied" 1.0 0 -6459832 true "" "plot count patches with [pcolor = brown]"
